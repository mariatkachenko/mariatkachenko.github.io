...
export default function AutopayProjectCard({ onOpen, ariaLabel, language, loadArtwork, disabled = false }: AutopayProjectCardProps) {
  return <button className="wallet-project-card autopay-project-card" type="button" data-interaction-sound="open" disabled={disabled} onClick={onOpen} aria-label={ariaLabel}>
    ...
    <span className="wallet-project-card__footer">
      ...
      <span className="wallet-project-card__copy">
        <strong className="wallet-project-card__title">{language === 'ru' ? 'Автоплатежи МТС' : 'MTS Autopay'}</strong>
