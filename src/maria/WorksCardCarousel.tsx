    ...
    {Array.from({ length: WORKS_CARD_COUNT }, (_, index) => {
      ...
      return <article
        ...
        style={{
          ...
          '--works-entry-y-mobile': `${compact(Math.sign(offset) * Math.min(2, Math.abs(offset)) * 1.6)}svh`,
          '--works-row-rotate-x-mobile': `${-pose.rotateY}deg`,
          '--works-loop-y-mobile': `${compact(mobileLoopPose.y)}svh`,
          ...
          '--works-deck-y-mobile': `${mobileDeckPose.y}svh`,
