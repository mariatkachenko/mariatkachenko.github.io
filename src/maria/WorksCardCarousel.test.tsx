...
describe('continuous works row geometry', () => {
  it('wraps fractional positions and keeps a straight equal row', () => {
    ...
    expect(WORKS_PROJECT_INDEX).toBe(1)
    expect(WORKS_INITIAL_POSITION).toBe(WORKS_PROJECT_INDEX)
    expect(WORKS_SBP_INDEX).toBe(0)
    expect(WORKS_AUTOPAY_INDEX).toBe(2)
    expect(WORKS_CONNECTION_INDEX).toBe(3)
    expect(WORKS_TINNOTECH_INDEX).toBe(4)
    expect(WORKS_ALIEXPRESS_INDEX).toBe(5)
    expect(WORKS_RARIBLE_INDEX).toBe(6)
    expect(WORKS_WALLET_INDEX).toBe(7)
    expect(WORKS_MTS_PLACEHOLDER_INDEX).toBe(8)
    ...
    expect(WORKS_CLICK_SCROLL_MS_PER_CARD).toBe(768)
    ...
    expect(worksPatternOffset(3.5)).toEqual({ desktopX: 1.543, desktopY: -0.613, mobileX: 0.514, mobileY: -1.379 })
    expect(worksPatternOffset(7)).toEqual({ desktopX: -2.364, desktopY: 0.139, mobileX: -0.788, mobileY: 0.313 })
    expect(normalizeWorksPosition(-0.5)).toBe(8.5)
    expect(continuousWorksOffset(0, 1.5)).toBe(-1.5)
    expect(continuousWorksOffset(13, 0.5)).toBe(3.5)
    expect(worksPositionAfterDelta(0, -300, 150)).toBe(2)
    expect(worksDragReleasePosition(6.375)).toBe(6.375)
    expect(worksDragReleasePosition(14.25)).toBe(5.25)
    ...
    expect(worksDragReleasePosition(13.7, WORKS_CARD_COUNT, true)).toBe(5)
    ...
    expect(mobileWorksLoopPose(3.5, WORKS_CARD_COUNT).y).toBeCloseTo(14.142)
    expect(mobileWorksLoopPose(3.5, WORKS_CARD_COUNT).scale).toBe(1)
    expect(mobileWorksLoopPose(5, WORKS_CARD_COUNT)).toEqual({ y: -7.524443153164711, scale: 1, opacity: 0.20472602547777755, layer: 2 })
    expect(mobileWorksLoopPose(-3.5, WORKS_CARD_COUNT).y).toBeCloseTo(-14.142)
  ...
  it('returns exactly five visible cards at integer and fractional positions', () => {
    expect([...visibleWorksCardIndices(1)]).toEqual([8, 0, 1, 2, 3])
    expect([...visibleWorksCardIndices(1.49)]).toEqual([8, 0, 1, 2, 3])
    expect([...visibleWorksCardIndices(1.5)]).toEqual([0, 1, 2, 3, 4])
    expect(visibleWorksCardIndices(8.8).size).toBe(5)
  })
})
...
describe('WorksCardCarousel', () => {
  ...
  it('reports a centered card only when the rounded center changes', () => {
    ...
    try {
      ...
      expect(onCenteredIndexChange).toHaveBeenLastCalledWith(WORKS_PROJECT_INDEX)
  ...
  it('renders equal structured cards without defective physical depth strips', () => {
    ...
    expect(cards).toHaveLength(WORKS_CARD_COUNT)
    ...
    expect(carousel).toHaveAttribute('data-works-position', '1')
    expect(cards[WORKS_PROJECT_INDEX]).toHaveAttribute('data-offset', '0')
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveAttribute('data-offset', '1')
    expect(cards[WORKS_CONNECTION_INDEX]).toHaveAttribute('data-offset', '2')
    expect(cards[WORKS_TINNOTECH_INDEX]).toHaveAttribute('data-offset', '3')
    expect(cards[WORKS_ALIEXPRESS_INDEX]).toHaveAttribute('data-offset', '4')
    expect(cards[WORKS_SBP_INDEX]).toHaveAttribute('data-offset', '-1')
    expect(cards[WORKS_MTS_PLACEHOLDER_INDEX]).toHaveAttribute('data-offset', '-2')
    expect(cards[WORKS_WALLET_INDEX]).toHaveAttribute('data-offset', '-3')
    ...
    expect(container.querySelectorAll('.works-project-card__image')).toHaveLength(0)
    ...
    expect(cards[WORKS_RARIBLE_INDEX].querySelectorAll('.rarible-project-card__cover')).toHaveLength(0)
    expect(cards[WORKS_RARIBLE_INDEX].querySelectorAll('.rarible-project-card__ape')).toHaveLength(0)
    expect(cards[WORKS_RARIBLE_INDEX].querySelectorAll('.rarible-project-card__logo')).toHaveLength(0)
    ...
    expect(cards[WORKS_AUTOPAY_INDEX].querySelectorAll('.autopay-project-card__phones')).toHaveLength(1)
    ...
    expect(cards[WORKS_SBP_INDEX].querySelectorAll('.sbp-project-card__phones')).toHaveLength(1)
    expect(cards[WORKS_CONNECTION_INDEX]).toHaveClass('has-connection')
    expect(cards[WORKS_CONNECTION_INDEX].querySelector('.connection-project-card')).not.toBeNull()
    expect(cards[WORKS_CONNECTION_INDEX].querySelectorAll('.connection-project-card__phones')).toHaveLength(1)
    expect(cards[WORKS_ALIEXPRESS_INDEX]).toHaveClass('has-aliexpress')
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelector('.aliexpress-project-card')).not.toBeNull()
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelectorAll('.aliexpress-project-card__phones')).toHaveLength(0)
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelectorAll('.aliexpress-project-card__bag')).toHaveLength(0)
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelectorAll('.aliexpress-project-card__heart')).toHaveLength(0)
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelectorAll('.aliexpress-project-card__sparkles')).toHaveLength(0)
    ...
    expect(container.querySelectorAll('.maria-works-deck-card.is-hidden')).toHaveLength(4)
    ...
    expect(container.querySelectorAll('.works-project-card__file-icon')).toHaveLength(0)
    ...
    expect(container.querySelectorAll('img[src="/assets/maria/mts-hanging-flag.webp"]')).toHaveLength(0)
    ...
    expect(cards[WORKS_PROJECT_INDEX].querySelectorAll('img[src="/assets/maria/mts-pay-card-composition-crisp.webp"]')).toHaveLength(1)
    ...
    expect(cards[WORKS_PROJECT_INDEX].querySelectorAll('img[src="/assets/maria/mts-pay-butterfly-flyout.webp?v=4"]')).toHaveLength(0)
    expect(cards[WORKS_ALIEXPRESS_INDEX].querySelector('img[src="/assets/maria/aliexpress-bag.webp"]')).toBeNull()
    expect(container.querySelectorAll('.works-project-card__breakout-artwork')).toHaveLength(0)
    expect(cards[WORKS_WALLET_INDEX].querySelector('.works-project-card__mts-flag')).toBeNull()
    expect(cards[WORKS_MTS_PLACEHOLDER_INDEX].querySelector('.works-project-card__mts-flag')).toBeNull()
    ...
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveStyle({ '--works-card-brightness-light': '0.92' })
    expect(cards[WORKS_PROJECT_INDEX]).toHaveStyle({ '--works-card-dark-glow': '0.18' })
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveStyle({ '--works-card-dark-hue': '-3deg' })
    expect(cards[WORKS_CONNECTION_INDEX]).toHaveStyle({ '--works-card-saturation-dark': '0.92' })
    ...
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveStyle({ '--works-card-shade-desktop-angle': '270deg' })
    expect(cards[WORKS_PROJECT_INDEX - 1]).toHaveStyle({ '--works-card-shade-mobile-angle': '180deg' })
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveStyle({ '--works-card-shade-mobile-angle': '0deg' })
    expect(cards[WORKS_PROJECT_INDEX - 1]).toHaveStyle({ '--works-card-mobile-lower-depth': '0' })
    expect(cards[WORKS_AUTOPAY_INDEX]).toHaveStyle({ '--works-card-mobile-lower-depth': '0.008' })
    expect(cards[WORKS_CONNECTION_INDEX]).toHaveStyle({ '--works-card-mobile-lower-depth': '0.002' })
    ...
    expect(cards[0].style.getPropertyValue('--works-loop-y-mobile')).toContain('dvh')
    ...
    expect(cards[0].style.getPropertyValue('--works-deck-y-mobile')).toContain('dvh')
  ...
  it('keeps distant heavy artwork deferred on the initial carousel view', () => {
    const { container } = render(<WorksCardCarousel onOpen={vi.fn()} language="ru" />)
    ...
    expect(container.querySelectorAll('img[src="/assets/maria/mts-pay-card-composition-crisp.webp"]')).toHaveLength(1)
    expect(container.querySelectorAll('img[src="/assets/maria/works-placeholder-payments-a.webp"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/works-placeholder-payments-b.png"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/aliexpress-bag.webp"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/aliexpress-collections-cover.webp"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/aliexpress-heart.webp"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/aliexpress-sparkles.webp"]')).toHaveLength(0)
    expect(container.querySelectorAll('img[src="/assets/maria/rarible-charity-cover.webp"]')).toHaveLength(0)
    ...
    expect(container.querySelectorAll('img[src="/assets/maria/mts-pay-butterfly-flyout.webp?v=4"]')).toHaveLength(0)
  ...
  it('loads the deferred MTS game layers one position before the card reaches center', () => {
    ...
    try {
      ...
      for (let step = 0; step < WORKS_CARD_COUNT - 2; step += 1) {
        act(() => vi.advanceTimersByTime(WORKS_AUTOPLAY_MS))
      }
  ...
  it('unmounts heavy artwork only after a project leaves the five-card visible window', () => {
    ...
    try {
      ...
      expect(container.querySelectorAll('.aliexpress-project-card__artwork img')).toHaveLength(0)
  ...
  it('renders localized English project-card metadata', () => {
    ...
    expect(screen.getAllByText('MTS Fintech 2026').length).toBeGreaterThanOrEqual(1)
  ...
  it('follows desktop drag continuously and settles the nearest card on release', () => {
    ...
    expect(carousel).toHaveAttribute('data-works-position', '3')
    expect(cards[3]).toHaveClass('is-centered')
    expect(container.querySelectorAll('.maria-works-deck-card.is-centered')).toHaveLength(1)
  })
  ...
  it('opens the QR payment presentation from its project card', () => {
    ...
    try {
      ...
      const sideQrButton = cards[WORKS_SBP_INDEX].querySelector<HTMLButtonElement>('button')
      ...
      expect(cards[WORKS_SBP_INDEX]).not.toHaveAttribute('inert')
      expect(cards[WORKS_SBP_INDEX].querySelector('.maria-works-deck-card__content')).not.toHaveAttribute('inert')
      expect(sideQrButton).not.toBeDisabled()
  ...
  it('moves a clicked outer visible card to the center without opening it', () => {
    ...
    try {
      ...
      fireEvent.pointerDown(cards[WORKS_CONNECTION_INDEX], { pointerId: 19, clientX: 700 })
      fireEvent.pointerUp(cards[WORKS_CONNECTION_INDEX], { pointerId: 19, clientX: 700 })
